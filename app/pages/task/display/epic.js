...
const updateTask = (action$, store, { client }) =>
  action$.ofType(types.UPDATE_TASK).mergeMap(action =>
    client({
      method: 'PUT',
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}`,
      entity: {
        id: action.taskId,
...
const fetchComments = (action$, store, { client }) =>
  action$.ofType(types.FETCH_COMMENTS).mergeMap(action =>
    client({
      method: 'GET',
      path: `${action.url}`,
      headers: {
...
const fetchTask = (action$, store, { client }) =>
  action$.ofType(types.FETCH_TASK).mergeMap(action =>
    client({
      method: 'GET',
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}?includeVariables=true`,
      headers: {
        Accept: 'application/json',
        Authorization: `Bearer ${store.getState().keycloak.token}`,
      },
...
const fetchCreateCommentForm = (action$, store, { client }) =>
  action$.ofType(types.FETCH_CREATE_COMMENT_FORM).mergeMap(() =>
    client({
      method: 'GET',
      ...
      .catch(error =>
        errorObservable(actions.fetchCreateCommentFormFailure(), error),
      ),
  );

const createComment = (action$, store, { client }) =>
  action$.ofType(types.CREATE_COMMENT).mergeMap(action =>
    client({
      method: 'POST',
      entity: {
...
const claimTask = (action$, store, { client }) =>
  action$.ofType(types.CLAIM_TASK).mergeMap(action =>
    client({
      method: 'POST',
      entity: {},
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}/_claim`,
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `Bearer ${store.getState().keycloak.token}`,
...
const unclaimTask = (action$, store, { client }) =>
  action$.ofType(types.UNCLAIM_TASK).mergeMap(action =>
    client({
      method: 'POST',
      entity: {},
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}/_unclaim`,
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `Bearer ${store.getState().keycloak.token}`,
...
const completeTask = (action$, store, { client }) =>
  action$.ofType(types.COMPLETE_TASK).mergeMap(action =>
    client({
      method: 'POST',
      entity: {},
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}/_complete`,
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `Bearer ${store.getState().keycloak.token}`,
