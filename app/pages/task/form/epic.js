...
const customEvent = (action$, store, { client }) =>
  action$.ofType(types.TASK_CUSTOM_EVENT).mergeMap(action =>
    client({
      method: 'POST',
      headers: {
      ...
      .map(payload => {
        ...
        const message =
          properties && properties['success-message']
            ? properties['success-message']
            : `${action.event.component.label} successfully actioned`;
      ...
      .catch(error =>
        errorObservable(actions.customEventFailure(error, action.event), error),
      ),
  );

const fetchTaskForm = (action$, store, { client }) =>
  action$.ofType(types.FETCH_TASK_FORM).mergeMap(action =>
    client({
      method: 'GET',
      path: `${store.getState().appConfig.formUrl}/form/name/${action.task.get(
        'formKey',
      )}`,
      headers: {
        Accept: 'application/json',
...
const submitTaskForm = (action$, store, { client }) =>
  action$.ofType(types.SUBMIT_TASK_FORM).mergeMap(action => {
    const data = {
      variables: {},
    };
    ...
    return client({
      ...
      path: `${
        store.getState().appConfig.workflowServiceUrl
      }/api/workflow/tasks/${action.taskId}/form/_complete`,
      entity: data,
      headers: {
        Accept: 'application/json',
        Authorization: `Bearer ${store.getState().keycloak.token}`,
