...
const fetchNotificationsNextPage = (action$, store, { client }) => action$.ofType(types.FETCH_NOTIFICATIONS_NEXT_PAGE)
...
const acknowledgeNotification = (action$, store, { client }) => action$.ofType(types.ACKNOWLEDGE_NOTIFICATION)
  .mergeMap(action => client({
    method: 'DELETE',
