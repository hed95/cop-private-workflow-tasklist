...
describe('Tasks reducer', () => {
  it('fetches tasks assigned to you', () => {
    ...
    expect(expected.getIn(['yourTasks', 'isFetchingTasksAssignedToYou']))
      .toEqual(true);
    expect(expected.getIn(['yourTasks', 'yourTasksFilterValue'])).toEqual('ABC');
    expect(expected.getIn(['yourTasks', 'yourTasksSortValue'])).toEqual('sort=due,desc');
  });
  it('successfully fetches tasks assigned to you', () => {
    ...
    expect(expected.getIn(['yourTasks', 'isFetchingTasksAssignedToYou'])).toEqual(false);
    expect(expected.getIn(['yourTasks', 'total'])).toEqual(1);
    expect(expected.getIn(['yourTasks', 'tasks']).size).toEqual(1);
    expect(expected.getIn(['yourTasks', 'yourTasksFilterValue'])).toBeNull();
    expect(expected.getIn(['yourTasks', 'yourTasksSortValue'])).toEqual('sort=due,desc');
