...
function reducer(state = initialState, action) {
  ...
  let processStartSort;
  let results;
  let updatedResults;
  switch (action.type) {
    case actions.FIND_CASES_BY_KEY:
      return state
        .set('businessKeyQuery', action.key)
        .set('searching', true)
    ...
    case actions.FIND_CASES_BY_KEY_SUCCESS:
      if (action.payload.entity && action.payload.entity._embedded) {
        caseResults = action.payload.entity._embedded
          ? action.payload.entity._embedded.cases
          : [];
    ...
    case actions.GET_CASE_BY_KEY:
      return state
        .set('businessKey', action.key)
        .set('loadingCaseDetails', true);
    case actions.GET_CASE_BY_KEY_SUCCESS:
      caseLoaded = action.payload.entity;
      caseLoaded.processInstances = _.orderBy(
        caseLoaded.processInstances,
        instance => {
    ...
    case actions.GET_FORM_SUBMISSION_DATA_SUCCESS:
      return state
        .set('loadingFormSubmissionData', false)
    ...
    case actions.LOAD_NEXT_SEARCH_RESULTS_SUCCESS:
      results = action.payload.entity._embedded
        ? action.payload.entity._embedded.cases
        : [];
      caseSearchResults = state.get('caseSearchResults');

      ({ cases } = caseSearchResults._embedded);
      updatedResults = _.unionBy(cases, results, 'businessKey');

      caseSearchResults._embedded.cases = sortByBusinessKey(updatedResults);
      caseSearchResults._links = action.payload.entity._links;
      return state
        .set('loadingNextSearchResults', false)
    ...
    case actions.RESET_FORM:
      return state
        .set('formSubmissionData', null)
        .set('formVersionDetails', null);

    case actions.SET_PROCESS_START_DATE_SORT:
      processStartSort = action.sort;
      caseDetails = state.get('caseDetails');

      caseDetails.processInstances = _.orderBy(
