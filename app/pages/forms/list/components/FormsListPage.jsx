                ...
                <div className="govuk-grid-column-full">
                    {isFetchingProcessDefinitions ? <h4 className="govuk-heading-s" id="loading">Loading forms...</h4> :
                        processDefinitions.map(p => {
                            const processDefinitionKey = p.getIn(['process-definition', 'key']);
                            const description = p.getIn(['process-definition', 'description']);
                            return <div id="form" className="govuk-grid-row" key={processDefinitionKey}>
                                <div className="govuk-grid-column-full govuk-card">
                                    <p id="formDescription" className="govuk-body govuk-body govuk-!-font-size-19">{description}</p>
                                    <div className="govuk-grid-row" key={processDefinitionKey}>
                                        <div className="govuk-grid-column-one-third">
                                            <button id="actionButton" className="govuk-button govuk-button--start"
                                                    onClick={() => this.process(p)} type="submit">Start
                                                <svg className="govuk-button__start-icon"
                                                     xmlns="http://www.w3.org/2000/svg" width="17.5" height="19"
